...
# --- Global Configuration / Constants (for consistency) ---
DEFAULT_MAX_VOCAB_SIZE = 15000 
DEFAULT_EMBEDDING_DIM = 128
DEFAULT_DROPOUT_RATE = 0.5
DEFAULT_EPOCHS = 10
DEFAULT_BATCH_SIZE = 32
DEFAULT_LEARNING_RATE = 0.001
DEFAULT_WINDOW_SIZE = 512 # Default window size for CNN input
...
def get_document_text(file_path):
    ...
    Currently supports plain text files (.txt) and PDF files (.pdf).
    Args:
    ...
    if file_extension == '.txt':
        ...
        # Example (requires PyPDF2 library):
        try:
            ...
            raise ImportError("PyPDF2 is not installed. Please install it to process PDFs (`pip install PyPDF2`).")
        except Exception as e:
            raise IOError(f"Error reading PDF {file_path}: {e}")
...
def build_cnn_model(vocab_size, embedding_dim, window_size, num_classes, dropout_rate=DEFAULT_DROPOUT_RATE, filters=128, kernel_size=5):
    """
...
def _train_folder_cnn_for_node(current_data_node_path, model_output_path, all_documents_data, args):
    ...
    # Build and train CNN
    model = build_cnn_model(vocab_size, args.embedding_dim, args.window_size, num_classes, args.dropout_rate)
    ...
    # Save CNN specific metadata (vocab_size, window_size, stride)
    cnn_metadata = {
...
def _prepare_decision_network_data(current_data_node_path, current_model_node_path, root_data_dir, model_base_dir, all_documents_data, args):
    ...
    child_cnn_metadata_loaded = {} # To store window_size, stride for each child CNN
    ...
    for subfolder_name in immediate_subfolders:
        ...
        if os.path.exists(child_model_path) and os.path.exists(child_tokenizer_path) and os.path.exists(child_metadata_path):
            try:
                ...
                child_cnn_metadata_loaded[subfolder_name] = {'window_size': 0, 'stride': 0, 'vocab_size': 0} # Mark as invalid
        else:
            child_cnn_metadata_loaded[subfolder_name] = {'window_size': 0, 'stride': 0, 'vocab_size': 0} # Mark as invalid
    ...
    for full_doc_path, doc_data in all_documents_data.items():
        ...
        if true_immediate_child_name in immediate_subfolders:
            ...
            dn_labels.append(f'retain_{true_immediate_child_name}') 
    ...
    dn_one_hot_labels = tf.keras.utils.to_categorical(dn_integer_labels, num_classes=len(dn_label_encoder.classes_))
...
def _predict_single_document_hierarchically(doc_text, model_base_dir, current_model_node_path, root_data_dir):
    ...
    if os.path.exists(decision_model_path):
        ...
        elif chosen_decision_label.startswith('retain_'):
            # It's a final classification at this level
            return [os.path.basename(current_model_node_path), chosen_decision_label.replace('retain_', '')]
        else:
